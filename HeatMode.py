...
class Confort_mode(object):
    ...
    def make_hot(self):
        new_mode = self._mode_list[min(
            self._current_index + 1, len(self._mode_list) - 1)]
...
class HeatMode(object):
    ...
    def __init__(self, outPlusWaveform=19, outMinusWaveform=12,
                 stateDisplayer=HeatingStateDisplayer()):
   ...
   # Set the pilot wire to confort mode = no sinusoid
    ...
    # Set the pilot wire to off mode = half positive sinusoid
    def setOffMode(self):
        self.sequencer.cancel()
        self._set_off_mode()
        self._displayer.displayOffMode()
    ...
    def _set_off_mode(self):
        self._setOutputs(plus=GPIO.HIGH, minus=GPIO.LOW)
